...
`mocode-floem-demo` uses the same `mocode-api` facade and renders line numbers, virtualized YAML rows, completion items, completion popup anchor, hover documentation, current YAML path, selection summary, and diagnostics from shared core state. It now supports the first editable loop: focus the editor surface, type text, use Backspace/Delete, move left/right, use Shift+Left/Right keyboard selection, copy selection with Ctrl/Cmd+C, paste text, insert committed IME text, and switch built-in fixtures from the header. IME preedit display is still later prototype work.
